...
class Proc(object):
    ...
    sources: dict
       ...
       Default: empty dict
    out_to_in: dict
        The keys are out_stream names, and the values are lists.
        out_to_in[out_stream_name] is a list of pairs:
                     (receiver_process_name, in_stream_name)
        where the out_stream called out_stream_name is connected to
        the in_stream called in_stream_name in the receiver process
        called receiver_process_name.
        process.
    ...
    def __init__(self, spec, connections, name):
        ...
        self.in_to_buffer = {}
        # process_id is a unique integer id of this process. This id
        # is used in termination detection.
...
class Proc(object):
    """
    Proc is a process in a multicore application.
...
def copy_buffer_segment(message, out_stream, buffer, in_stream_type):
    ...
    if end >= start:
        ...
        remaining_space = BUFFER_SIZE - start
        segment_length = remaining_space + end
        # Set up a multiprocessing.Array with appropriate length
        # The array will be filled in later.
        return_value = multiprocessing.Array(in_stream_type, range(segment_length))
        return_value[:remaining_space] = \
            multiprocessing.Array(in_stream_type, buffer[start:])
        return_value[remaining_space:] = \

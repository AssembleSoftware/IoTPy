...
def single_process_single_source_example_1():
    ...
    def source(out_stream):
        ...
        # Return an agent which takes num_steps steps, and
        # sleeps for time_interval seconds between successive steps,
        # and puts the next element generated by the agent in stream
        # out_stream, and starts the generating function, func, with
        # initial state of 0.
        # The elements on out_stream will be 1, 2, 3, ..., num_steps
...
def single_process_multiple_sources_example_1():
    ...
    def compute_func(in_streams, out_streams):
        ...
        x_stream, y_stream = in_streams
        stream_to_file(in_stream=x_stream, filename='x.dat')
        stream_to_file(in_stream=y_stream, filename='y.dat')
        t = Stream()
...
def simple_actuator_example():
    ...
    num_steps = 10
    # STEP 1: DEFINE SOURCES
    def sequence_of_integers(current_integer):
    ...
    def sequence_of_integers_source(out_stream):
        return source_func_to_stream(
            ...
            num_steps=num_steps, window_size=1, state=0)
...
def actuator_example_2():
    ...
    proc=shared_memory_process(
        ...
        in_stream_names=['in'],
        out_stream_names=['out'],
        connect_sources=[('in', source_list.source_func)],
        connect_actuators=[('out', print_from_queue)]
        )
    vm = Multiprocess(processes=[proc], connections=[])
    vm.run()

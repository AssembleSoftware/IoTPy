...
data from a data source, such as a sensor, into a stream; it does this
with the help of the scheduler thread. Each source function is
executed in a separate thread. The source thread puts values
generated by a function func in a queue called scheduler.input_queue.
The scheduler appends data it gets from its input queue on to a
stream. The destination stream is identified by its name which is a
parameter of source. 

A function returns a 2-tuple: (1) thread and (2) ready, a
threading.Event. Executing ready.wait() on any thread causes the
calling thread to wait until the source thread is ready for
execution.

Functions in the module:
   1. source_function: puts data generated by a function into a stream.
   the queue read by the scheduler.
   2. source_file: puts data from a file into the queue read by the
   scheduler.
   3. source_list: puts data from a list into the queue read by the
   scheduler.

Legacy functions (not used)
   1. func_to_q: Puts results of a function on to any specified queue.
   2. q_to_streams: Puts data from a queue onto one or more streams
   3. q_to_streams_general: General verson of q_to_streams in which
   the destination streams can be specified by any object.

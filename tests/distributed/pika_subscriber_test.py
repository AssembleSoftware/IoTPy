...
from IoTPy.concurrency.PikaSubscriber import pika_callback_thread_target
...
def pika_subscriber_test():
    ...
    def pika_receive_agent(in_streams, out_streams):
        print_stream(in_streams[0], 'pika_receive_stream')

    # Step 0.1: Define source thread targets (if any).
    # Defined in pika_callback_thread_target imported from PikaSubscriber
    ...
    pika_callback_thread = threading.Thread(
        target=pika_callback_thread_target,
        args=(procs, 'pika_receive_stream', 'temperature', 'publications', 'localhost'))
